...
MovePiranhaPlant:
      ...
      bne PiranhaInitDone         ;if set at all, branch to leave
      lda EnemyFrameTimer,x       ;check enemy's timer here
      bne PiranhaInitDone         ;branch to end if not yet expired
...
ChkPlayerNearPipe:
      ...
      cmp #$21
      bcc PiranhaInitDone         ;if player within a certain distance, branch to leave
...
RiseFallPiranhaPlant:
      ...
      lsr
      bcc PiranhaInitDone         ;branch to leave if d0 set (execute code every other frame)
      lda TimerControl            ;get master timer control
      bne PiranhaInitDone         ;branch to leave if set (likely not necessary)
      ...
      bne PiranhaInitDone         ;branch to leave if not yet reached
      lda #$00
